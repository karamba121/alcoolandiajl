      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            sh(30),
            customTextInput(color, rcolor,'title', _title),
            customTextInput(
                color, rcolor, 'description', _description),
            customTextInput(color, rcolor, 'price', _price),
            customTextInput(color, rcolor, 'height', _height),
            customTextInput(
                color, rcolor,'temperature', _temperature),
            customTextInput(color, rcolor, 'pot', _pot),
            sh(dh(context) / 5)
          ],
        ),
      ),
      ...
      floatingActionButton: Container(
        ...
        child: GestureDetector(
          onTap: () async {
            ...
            // DocumentReference plantId = kFirestore.collection('posts').doc();
            // var plants = await state.fetchPlants();
            // var plant = state.fetchPlant(plants.first.data()!);
            // plant.plantId = plantId.id;
            // await state.salePlant(plant);
            // Navigator.pop(context);
